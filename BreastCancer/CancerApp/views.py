...
def UploadFileAction(request):
    if request.method == 'POST':
        ...
        with open("CancerApp/static/test.jpg", "wb") as file:
            file.write(myfile)
        file.close()
        img,pres = predict("CancerApp/static/test.jpg", extension_model)
        var=""
        if pres=="Benign":
            var="cancer detected please follow up with doctor"
        if pres=="Malignant":
            var="cancer detected please consult doctor"
        if pres=="Normal":
            var="please be safe"
        plt.imshow(img)
        buf = io.BytesIO()
